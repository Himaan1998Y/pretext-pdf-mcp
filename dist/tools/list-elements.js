...
## qr-code
Generates a QR code image. Requires the \`qrcode\` peer dependency. Key props: \`data\` (string to encode, required), \`size\` (pt, default 80), \`errorCorrectionLevel\` (L|M|Q|H), \`foreground\`, \`background\` (#hex), \`margin\`, \`align\`.
Example: \`{ type: "qr-code", data: "https://example.com", size: 100, align: "center" }\`

## barcode
Generates a barcode using bwip-js. Requires the \`bwip-js\` peer dependency. Key props: \`symbology\` (e.g. 'code128', 'ean13', 'qrcode', required), \`data\` (required), \`width\` (pt, default 200), \`height\` (pt, default 60), \`includeText\` (human-readable text below), \`align\`.
Example: \`{ type: "barcode", symbology: "code128", data: "ABC-12345", width: 180, height: 60 }\`

## chart
Renders a Vega-Lite chart as a rasterized image. Requires \`vega\` and \`vega-lite\` peer dependencies. Key props: \`spec\` (Vega-Lite JSON spec object, required), \`width\` (pt, default content width), \`height\` (pt, default 300), \`caption\`, \`align\`.
Example: \`{ type: "chart", spec: { mark: "bar", data: { values: [{ a: "A", b: 10 }] }, encoding: { x: { field: "a" }, y: { field: "b" } } }, height: 200 }\`

## toc-entry
Internal element auto-generated by the TOC two-pass processor. Do NOT add manually — use \`{ type: "toc" }\` instead and let the renderer place entries.
Example: N/A — internal use only.

## footnote-def
Defines a footnote that appears at the bottom of the page where its reference appears. Pair with \`InlineSpan.footnoteRef\` in a \`rich-paragraph\`. Key props: \`id\` (unique, required — matches the \`footnoteRef\` in a span), \`text\` (required), \`fontSize\`, \`fontFamily\`, \`spaceAfter\`. Use \`createFootnoteSet()\` from the library for convenient pairing.
Example: \`{ type: "footnote-def", id: "fn-1", text: "See Smith (2022) for details." }\`

## float-group
Places an image in a side column (left or right) with one or more paragraphs/headings flowing in the adjacent column. Key props: \`image\` ({ src, height? }, required), \`float\` ('left'|'right', required), \`content\` (array of paragraph/heading/rich-paragraph, required), \`floatWidth\` (pt, default 35% of content width), \`floatGap\` (pt, default 12), \`spaceAfter\`.
Example: \`{ type: "float-group", float: "left", image: { src: "/abs/path/logo.png" }, content: [{ type: "paragraph", text: "Caption text here." }], floatWidth: 120 }\`
...
`;
const _coveredTypes = ELEMENTS_REFERENCE.match(/^## (\S+)/gm)?.map((h) => h.slice(3)) ?? [];
const _missingDocs = ELEMENT_TYPES.filter(t => !_coveredTypes.includes(t));
