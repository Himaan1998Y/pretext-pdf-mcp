...
export function formatMoney(amount: number, symbol: string, currency: SupportedCurrency): string {
  const locale = CURRENCY_LOCALES[currency]
  return `${symbol}${amount.toLocaleString(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
}

export function partyBlock(p: InvoiceParty): string {
...
export function buildInvoiceDocument(input: InvoiceInput, invoiceNo: string): PdfDocument {
  const currency = input.currency ?? 'INR'
  const sym = CURRENCY_SYMBOLS[currency]
  const date = input.date ?? todayISO()
  const hasHsn = input.items.some((i: InvoiceItem) => i.hsn_code)
