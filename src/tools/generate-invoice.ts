...
import { type InvoiceParty, type InvoiceItem, type InvoiceInput, SUPPORTED_CURRENCIES } from './invoice/types.js'
...
export const generateInvoiceTool = {
  schema: {
    ...
    description:
      `Generate a professional invoice PDF. Accepts structured invoice data (from/to parties, line items, optional GST/tax). Returns base64-encoded PDF. Supports ${SUPPORTED_CURRENCIES.join('/')} currencies. Currency symbols (₹ $ € £) are guaranteed not to break away from adjacent numbers across line wraps. gst_rate on items creates a tax column — use it for any tax system (GST, VAT, sales tax). Per-item gst_rate is summed as IGST in the totals; CGST/SGST inter-vs-intra-state routing is not currently supported.`,
    inputSchema: {
  ...
  handler: async (args: Record<string, unknown>) => {
    try {
      ...
      if (typeof args.notes === 'string' && CTRL_CHARS.test(args.notes as string)) {
